...
if len(sys.argv) < 4:
  print "usage: ab_submit.py <coordinator hostname> <web UI port> " \
...
if len(sys.argv) > 6:
  ...
  job_name = "ab_job_at_%d" % (int(time.time()))

basic_args = []
basic_args.append("/home/srguser/firmament-experiments/helpers/napper/napper_ab.py")
...
job_desc.root_task.args.append(ab_exe)
...
for i in range(1, num_workers):
  ...
  add_worker_task(job_name, task, ab_exe, basic_args, i, num_workers, extra_args)

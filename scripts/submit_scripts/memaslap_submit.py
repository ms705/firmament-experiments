...
if len(sys.argv) < 4:
  print "usage: memaslap_submit.py <coordinator hostname> <web UI port> " \
      "<task binary> [<args>] [<num workers>] [<job name>]"
  sys.exit(1)

hostname = sys.argv[1]
port = int(sys.argv[2])
memaslap_exe = sys.argv[3]
...
if len(sys.argv) > 6:
  ...
  job_name = "memaslap_job_at_%d" % (int(time.time()))

basic_args = []
basic_args.append("/home/srguser/firmament-experiments/helpers/napper/napper_memaslap.py")
...
job_desc.root_task.args.append(memaslap_exe)
...
for i in range(1, num_workers):
  ...
  add_worker_task(job_name, task, memaslap_exe, basic_args, i, num_workers, extra_args)

...
if len(sys.argv) < 4:
  print "usage: kittydar_submit.py <coordinator hostname> <web UI port> " \
      "<task binary> [<args>] [<job name>] [<num workers>]"
  sys.exit(1)

hostname = sys.argv[1]
port = int(sys.argv[2])
kittydar_exe = sys.argv[3]
...
if len(sys.argv) > 5:
  ...
  job_name = "kittydar_job_at_%d" % (int(time.time()))
...
basic_args = []
basic_args.append("/home/srguser/firmament-experiments/helpers/napper/napper_kittydar.py")
...
job_desc.root_task.args.append(kittydar_exe)
...
for i in range(1, num_workers):
  ...
  add_worker_task(task, kittydar_exe, basic_args, i, num_workers, extra_args)

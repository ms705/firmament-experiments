...
sleep 2
parallel-ssh -t 0 -h ~/caelum-200 -i "cd firmament; build/src/coordinator --parent_uri=tcp:$2:9999 --task_lib_dir=${ROOT}/firmament/build/src/ --scheduler=simple --log_dir=${LOG_DIR} --task_log_dir=${ROOT}/firmament-logs/ --task_perf_dir=${ROOT}/firmament-perf --task_data_dir=/mnt/scratch/ --pin_tasks_to_cores=true --preemption=false --hdfs_name_node_address=hdfs://caelum10g-301.cl.cam.ac.uk --health_monitor_enable=false --heartbeat_interval=200000 --perf_monitoring=false --monitor_netif=p1p1" &

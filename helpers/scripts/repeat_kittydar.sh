...
until [[ ${CUR_TIME} -gt $(expr ${START_TIME} + ${RUN_TIME}) ]]; do
  JOB_ID=$(python kittydar_submit.py localhost 8080 /home/srguser/firmament-experiments/workloads/kittydar/testing/test.js " --count=20" kittydar${i}_run${j} ${i} | grep "JOB ID" | cut -d' ' -f4)

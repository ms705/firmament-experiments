...
if [[ ${CONF} == "server" ]]; then
  ${DIR}/perf_helper.sh /usr/bin/time -o ${OUT_DIR}/net_stream.${CONF}.${HOSTNAME}${EXPERIMENT}.time --verbose /usr/bin/iperf -s ${SERVER} -o ${OUT_DIR}/io_stream.${CONF}.${HOSTNAME}${EXPERIMENT}.out -i 1
else

...
def plot_cdf(plot_file_name, cdf_vals, label_axis, labels, bin_width=1000):
    ...
    for vals in cdf_vals:
        ...
        bins = numpy.linspace(0, 160, 80)
        plt.hist([x / 1000.0 / 1000.0 for x in vals], bins, alpha=0.5,
                 label=labels[index], color=colors[labels[index]], edgecolor='none')
        index += 1
    ...
    plt.xlabel("Task response time [sec]")
    plt.ylabel("Number of tasks [$\\log_{10}$]")

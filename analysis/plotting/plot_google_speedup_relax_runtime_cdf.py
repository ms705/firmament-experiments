...
gflags.DEFINE_bool('log_scale', False, 'Plot on log scale')
gflags.DEFINE_string('time_unit', 'sec', 'Time unit: sec | ms')
...
def plot_cdf(plot_file_name, cdf_vals, label_axis, labels, log_scale=False,
    ...
    if log_scale:
        ...
        plt.xlim(0, max_cdf_val)
        plt.xticks(range(0, max_cdf_val, 200000000),
                   [str(x / to_time_unit) for x in range(0, max_cdf_val, 200000000)])
    plt.ylim(0.9, 1.0)
    plt.yticks(np.arange(0.95, 1.01, 0.01),
               [str(x) for x in np.arange(0.95, 1.01, 0.01)])
...
def main(argv):
    ...
    plot_cdf('google_speedup_runtime_cdf', algo_runtimes,
             'Algorithm runtime [' + FLAGS.time_unit + ']',
             algo_trace_labels, log_scale=FLAGS.log_scale, bin_width=1000,
             unit=FLAGS.time_unit)

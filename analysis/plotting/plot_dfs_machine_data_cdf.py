...
def get_data_per_machine(trace_path):
    ...
    for row in csv_reader:
        ...
        if machine_id in machine_data:
            machine_data[machine_id] = machine_data[machine_id] + block_size
        else:
...
#        print "Statistics for %s" % (labels[index])
    ...
    if log_scale:
        ...
        plt.xlim(0, max_cdf_val)
        plt.xticks(range(0, max_cdf_val, 100 * BYTES_TO_GB),
                   [str(x / BYTES_TO_GB) for x in range(0, max_cdf_val, 100 * BYTES_TO_GB)])
    plt.ylim(0, 1.0)
...
def main(argv):
    ...
    plot_cdf('dfs_machine_data_cdf',
             ...
             'Data per machine [GB]', [], log_scale=False,
             bin_width=BYTES_TO_GB)

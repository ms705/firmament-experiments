...
double gen_initial_optvals(void) {
  double stkval;
  for (int j=n; j>-1; j--) {
...
double process_rows(int rowstart, int rowto, double v) {
...
int main(int argc, char **argv)
{
  ...
  init_vals();
  double start = gen_initial_optvals();
  double v = start;
  for (int i = n; i > 0; i--) {
    int rowto = (i - chunk) > 0 ? (i - chunk) : 0;
    printf("processing rows %d to %d\n", i, rowto);
    process_rows(i, rowto, v);
  }
  printf("%.9f\n", v);
  return 0;
}
